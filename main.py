...
def back_to_main(user_type="user"):
    if user_type == "admin":
        kb = [
...
async def handle_message(message: types.Message, state: FSMContext):
    ...
    if text == "/start":
        if int(user_id) == ADMIN_ID:
            await message.answer("Привет, админ! Выберите действие 👇", reply_markup=back_to_main("admin"))
        else:
    ...
    if int(user_id) == ADMIN_ID:
        ...
        if current_state == AdminStates.add_category:
            if text in CATEGORIES:
                await message.answer("Категория уже существует.", reply_markup=back_to_main("admin"))
            else:
                CATEGORIES[text] = {}
                save_data()
                await message.answer(f"Категория '{text}' добавлена ✅", reply_markup=back_to_main("admin"))
            await state.clear()
            return
            ...
            if cat:
                CATEGORIES[cat][text] = []
                save_data()
                await message.answer(f"Подкатегория '{text}' добавлена в '{cat}' ✅", reply_markup=back_to_main("admin"))
            await state.clear()
            return
            ...
            save_data()
            await message.answer(f"Товар '{name}' добавлен в '{cat} -> {sub}' ✅", reply_markup=back_to_main("admin"))
            await state.clear()
            return
            ...
            try:
                ...
                if new_id not in managers:
                    ...
                    save_data()
                    await message.answer(f"Менеджер {new_id} добавлен ✅", reply_markup=back_to_main("admin"))
                else:
                    await message.answer("Менеджер уже существует.", reply_markup=back_to_main("admin"))
            except ValueError:
                await message.answer("Введите корректный числовой ID Telegram.", reply_markup=back_to_main("admin"))
            await state.clear()
            return
    ...
    if text == "🧺 Корзина":
        ...
        await message.answer(f"{items_text}\n\n💰 Итого: ${total}", reply_markup=back_to_main("user"))
        return
...
async def handle_callbacks(cb: types.CallbackQuery, state: FSMContext):
    ...
    if data_cb.startswith("delcat_"):
        ...
        if cat in CATEGORIES:
            del CATEGORIES[cat]
            save_data()
            await cb.message.answer(f"Категория '{cat}' удалена ✅", reply_markup=back_to_main("admin"))
        await cb.answer()
        return
    ...
    if data_cb.startswith("delsubcat_"):
        ...
        if not subs:
            await cb.message.answer("В данной категории нет подкатегорий.", reply_markup=back_to_main("admin"))
            await cb.answer()
            return
        kb = types.InlineKeyboardMarkup(
    ...
    if data_cb.startswith("delsub_"):
        ...
        if sub in CATEGORIES.get(cat, {}):
            ...
            save_data()
            await cb.message.answer(f"Подкатегория '{sub}' удалена из '{cat}' ✅", reply_markup=back_to_main("admin"))
        await cb.answer()
        return
    ...
    if data_cb == "back_main":
        if int(user_id) == ADMIN_ID:
            await cb.message.answer("Выберите действие 👇", reply_markup=back_to_main("admin"))
        else:
            await cb.message.answer("Выберите действие 👇", reply_markup=main_menu())
        await cb.answer()
        return
